...
void main() {
	vec4 clipPos = ubo.proj * ubo.view * ubo.model * vec4(inPosition, 1.0); // store clip-space position (before perspective divide)
	gl_Position = clipPos;
	v2fCol = inColor;
	v2fUV = inUV;
	...
	v2fWorldPos = vec3(ubo.model * vec4(inPosition, 1.0));
